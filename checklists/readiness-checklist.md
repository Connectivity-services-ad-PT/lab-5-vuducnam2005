...
- [x] **Image tags:** bạn đã build image với tag `v0.1.0-team-iot` và push lên registry (ghcr.io hoặc Docker Hub). Xác nhận rằng tag xuất hiện trong registry.

Ghi chú thêm những vấn đề gặp phải hoặc điều chỉnh tại đây:

```
- Sử dụng chung Dockerfile cho api và ai-service để tối ưu hoá build và quản lý dependencies thống nhất.
- Sử dụng psycopg2-binary kết nối PostgreSQL, lưu telemetry data thực tế vào database thay vì in-memory list.
- Thiết lập port forwarding đầy đủ 8000 (API), 9000 (AI) và 5432 (DB).
- Sửa lỗi mapping healthcheck bằng curl trên python-slim sang python urllib để tránh lỗi thiếu curl tool trong container.
```